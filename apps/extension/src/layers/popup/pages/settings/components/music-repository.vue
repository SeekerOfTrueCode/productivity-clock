...
const getMusicEntries = computed(() =>
  Object.entries(soundsAndMusicStore.music)
)
...
const {
  ...
  open (key: string) {
    ...
    const oldSound = soundsAndMusicStore.get(StorageType.Music, key)
    editKey.value = key
  ...
  async save ({ preventClosing }) {
    ...
    if (editSound) {
      ...
      soundsAndMusicStore.set(StorageType.Music, editKey.value!, {
        ...oldSound,
        name: name.value,
        uri: uri.value,
      ...
      soundsAndMusicStore.add(StorageType.Music, {
        name: name.value,
        uri: uri.value,
        sourceType: sourceType.value!
      })
...
<template>
  <v-table fixed-header density="compact" height="400px">
      ...
      <tr>
        <th class="text-left">
          <div class="d-flex align-center justify-start">
            {{
              $t(
                "settings-page.sounds-and-music-page.music-repository-header-name"
              )
            }}
        ...
        <th class="text-left">
          <div class="d-flex align-center justify-start">
            {{
              $t(
                "settings-page.sounds-and-music-page.music-repository-header-type"
              )
            }}
    ...
    <tbody>
      <tr
        v-for="[key, sound] in getMusicEntries"
        :key="key"
        v-ripple
      ...
      >
        ...
        <td class="text-left">
          <UiBtn
            ...
            size="small"
            icon
            @click.stop="soundsAndMusicStore.remove(StorageType.Music, key)"
          >
            <UiIcon>
              <icon-mdi-close />
            </UiIcon>
    ...
    <UiCard class="w-[400px]">
      <UiCardText>
        {{
          edit
            ? $t(
              "settings-page.sounds-and-music-page.music-repository-edit-music"
            )
            : $t(
              "settings-page.sounds-and-music-page.music-repository-add-new-music"
            )
        }}
      </UiCardText>
      <UiCardText>
  ...
  <EditAddButtonsFloating
    v-model:edit="edit"
    :edit-disabled="getMusicEntries.length === 0"
    :edit-tooltip="
      $t('settings-page.sounds-and-music-page.music-repository-edit-music')
    "
    :edit-finish-tooltip="
      $t('settings-page.sounds-and-music-page.music-repository-edit-finish')
    "
    :add-tooltip="
      $t('settings-page.sounds-and-music-page.music-repository-add-new-music')
    "
    @add="popupOpen"
  />
</template>
